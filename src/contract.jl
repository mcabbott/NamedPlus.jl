...
"""
contract(A::NamedDimsArray{LA}, B::NamedDimsArray{LB}) where {LA,LB} =
    contract(A, B, filter(n -> Base.sym_in(n, LB), LA))

contract(A::NamedDimsArray, B::NamedDimsArray, s::Symbol) =
    contract(A, B, (s,))

function contract(A::NamedDimsArray{LA,TA,NA}, B::NamedDimsArray{LB,TB,NB}, s::Tuple{Vararg{Symbol}}) where {LA,LB,TA,TB,NA,NB}
    oindA = filter(d -> !Base.sym_in(LA[d], s), ntuple(identity, NA))
    cindA = filter(d -> Base.sym_in(LA[d], s), ntuple(identity, NA))

    oindB = filter(d -> !Base.sym_in(LB[d], s), ntuple(identity, NB))
    cindB = filter(d -> Base.sym_in(LB[d], s), ntuple(identity, NB))

    indCinoAB = (ntuple(identity, length(oindA))..., ntuple(d -> d + length(oindA), length(oindB))...)
    sizeC = (map(d -> size(A,d), oindA)..., map(d -> size(B,d), oindB)...)

    TC = Base.promote_type(TA,TB)
    C = similar(nameless(A), TC, sizeC)
    contract!(true, nameless(A), :N, nameless(B), :N, false, C, oindA, cindA, oindB, cindB, indCinoAB)

    ndims(C) == 0 && return first(C)
